...
void write_bpheader(BPTree *T) {
    assert(T && T->f);
    if (fseeko(T->f, 0, SEEK_SET) != 0) {
        perror("fseeko write header");
        exit(1);
    }
    if (fwrite(&T->hdr, sizeof(BPHeader), 1, T->f) != 1) {
...
void node_serialize(BPTree *T, Node *node, uint8_t *pagebuf) {
    ...
    if (node->hdr.is_leaf) {
        ...
        for (int i = 0; i < node->hdr.n; i++) {
            memcpy(p, &node->keys[i].value, sizeof(int32_t));
            p += sizeof(int32_t);
            memcpy(p, &node->keys[i].id, sizeof(int64_t));
            p += sizeof(int64_t);
            memcpy(p, &node->ptrs[i], sizeof(int64_t));
            p += sizeof(int64_t);
        }
    } else {
        // internal: child_ptr0, then for i: key_i, child_ptr[i+1]
        // write child ptr 0
        memcpy(p, &node->ptrs[0], PTR_SIZE);
        p += PTR_SIZE;
        for (int i = 0; i < node->hdr.n; i++) {
            memcpy(p, &node->keys[i].value, sizeof(int32_t));
            p += sizeof(int32_t);
            memcpy(p, &node->keys[i].id, sizeof(int64_t));
            p += sizeof(int64_t);
            memcpy(p, &node->ptrs[i + 1], PTR_SIZE);
            p += PTR_SIZE;
        }
    }
}
...
void node_deserialize(BPTree *T, Node *node, const uint8_t *pagebuf) {
    ...
    if (node->hdr.is_leaf) {
        for (int i = 0; i < node->hdr.n; i++) {
            memcpy(&node->keys[i].value, p, sizeof(int32_t));
            p += sizeof(int32_t);
            memcpy(&node->keys[i].id, p, sizeof(int64_t));
            p += sizeof(int64_t);
            memcpy(&node->ptrs[i], p, sizeof(int64_t));
            p += sizeof(int64_t);
        }
    } else {
        memcpy(&node->ptrs[0], p, PTR_SIZE);
        p += PTR_SIZE;
        for (int i = 0; i < node->hdr.n; i++) {
            memcpy(&node->keys[i].value, p, sizeof(int32_t));
            p += sizeof(int32_t);
            memcpy(&node->keys[i].id, p, sizeof(int64_t));
            p += sizeof(int64_t);
            memcpy(&node->ptrs[i + 1], p, PTR_SIZE);
            p += PTR_SIZE;
        }
    }
}
...
int64_t find_leaf_page(BPTree *T, const Key *k, Node **leaf_out) {
    ...
    *leaf_out = NULL;
    ...
    while (1) {
        ...
        // Validate node size (corruption check)
        if (hdr.is_leaf) {
        ...
        // Internal node — choose child pointer
        int i = 0;
...
int64_t leaf_split(BPTree *T, Node *leaf, int64_t leaf_offset, const Key *k, int64_t value_offset, Key *promoted) {
    ...
    if (!tmp_keys || !tmp_vals) {
        perror("malloc tmp in leaf_split");
        exit(1);
    }
    ...
    *count_out = 0;
    ...
    if (*leaf_off_io == -1) {
        ...
        *pos_io = 0;
        ...
        // Subsequent call: continue from saved leaf
        leaf = read_node(T, *leaf_off_io);
        if (!leaf) return results;
    }

    // Scan leaves and gather at most 10 results
